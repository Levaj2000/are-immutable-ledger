...
export default function Home() {
  ...
  useEffect(() => {
    if (!auto || mode !== 'journey' || step >= journey.length - 1) return;
    const timer = setTimeout(() => {
      const next = step + 1;
      setStep(next);
      if (next >= journey.length - 1) setAuto(false);
    }, 2600);
