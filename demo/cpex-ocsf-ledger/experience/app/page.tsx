...
const businessJourney = [
  ...
  { phase:'DEATH AND REBIRTH', title:'The producer dies mid-stream', summary:'The process restarts. The new producer tells the truth about its new identity.', amount:0, balance:15, status:'RECOVERED', tone:'blue', detail:'Rev 2 verifies dense sequence within each epoch; no stamps are rewritten.', stream:'same stream · new epoch · seq 1', evidence:'Accepted Rev 2 recovery path' },
  { phase:'THE REWARD', title:'The evidence converges', summary:'Identity and run correlation join every decision across producer lifecycles.', amount:0, balance:15, status:'CORRELATED', tone:'green', detail:'Rev 2: agent-7 and run-4bf92f35 on all 6 · request ID on Beat 05 only', stream:'Rev 2: 1 stream · 2 epochs', evidence:'Rev 3 target: 2 producer streams' },
...
const technicalJourney = [
  ...
  { phase:'PANIC CONTAINMENT', title:'Real plugin_panic path', summary:'catch_unwind contains the plugin panic and the executor finalizes a fail-closed deny.', amount:0, balance:15, status:'DENY', tone:'amber', detail:'plugin_panic violation · terminal stop · DecisionLog reaches audit sink', stream:'Rev 3: stream_id=decision · fresh epoch', evidence:'Path live · Rev 3 bundle pending' },
  { phase:'STREAM RECOVERY', title:'Producer restarts honestly', summary:'Rev 2 proves restart handling: a new epoch and chain_uid with stream_seq reset to 1.', amount:0, balance:15, status:'DENSE', tone:'blue', detail:'density scope=(epoch,stream_id) · no cross-epoch continuity claim', stream:'Rev 2: same stream · new epoch', evidence:'Accepted · no rewritten stamps' },
  { phase:'CORRELATION', title:'Join across producer lifecycles', summary:'Agent and conversation correlation survive process and epoch boundaries in Rev 2.', amount:0, balance:15, status:'JOINED', tone:'green', detail:'ai_agent.uid=agent-7 · correlation_uid=run-4bf92f35 · request_id event-scoped', stream:'Rev 2: 1 stream · 2 epochs', evidence:'Rev 3 target: 2 streams' },
