    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
        <div style={{ flex: 1 }}>
          ...
          {step === 4 && <StepNarrative title="Gateway evaluates receipt policy" text="The MCP Gateway calls VerifyProof with the hash. It then validates AuthBridge's signature and trust chain, confirms the input_hash matches its current payload, checks freshness and the guardrail version, and skips the PII scan only if local policy accepts every result." color="var(--purple)" />}
          ...
          {step === 6 && <StepNarrative title="Server evaluates both receipts" text="The MCP Server evaluates both receipts independently: ledger-envelope integrity, issuer signature and trust chain, payload binding, freshness, check version, audience, and local authorization policy. A valid ledger hash alone is never treated as permission." color="var(--green)" />}
